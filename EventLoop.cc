    ...
    : looping_(false), quit_(false), callingPendingFunctors_(false), threadId_(CurrentThread::tid()), poller_(Poller::newDefaultPoller(this)), wakeupFd_(createEventfd()), wakeupChannel_(new Channel(this, wakeupFd_))
{
    ...
    {
        LOG_FATAL("Anotheer EventLoop %p exists in this thread %d \n", t_loopInThisThread, threadId_);
    }
    else
    {
...
void EventLoop::loop()
{
    ...
    while (!quit_)
    {
         ...
         * mainLoop 事先注册一个回调cb (需要subLoop执行)    wakeup subloop后 执行下面的方法 执行之前mainloop注册的回调
         */
        doPendingFunctors();
    }
...
{
    quit_ = true;
    // 如果是在其它线程中 调用quit 在一个subloop(worker)中 调用了mainloop(IO)的quit
    if (!isInLoopThread())
    {
        wakeup();
    }
...
// 用来唤醒loop所在的线程的 向wakeupfd写一个数据 wakeupChannel就发生读事件, 当前loop线程就会被唤醒
...
void EventLoop::doPendingFunctors()
{
    ...
    for(const Functor & funcotr:functors)
    {
        funcotr();  // 执行当前loop需要执行的回调操作
    }
